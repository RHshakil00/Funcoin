...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 FUN at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zfun send
enum ZerocoinSpendStatus {
    ZFUN_SPEND_OKAY = 0,                            // No error
    ZFUN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFUN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFUN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFUN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFUN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFUN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFUN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFUN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFUN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFUN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFUN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFUN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFUN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFUN_SPENT_USED_ZFUN = 14,                      // Coin has already been spend
    ZFUN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZFUN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZFUNBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZFUNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzfunAuto) const;
    ...
    */
    ...
    CzfunWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzfunTracker> zfunTracker;
    ...
    void setZWallet(CzfunWallet* zwallet)
    {
        zwalletMain = zwallet;
        zfunTracker = std::unique_ptr<CzfunTracker>(new CzfunTracker(strWalletFile));
    }

    CzfunWallet* getZWallet() { return zwalletMain; }
    ...
    /** zfun reset */
    boost::signals2::signal<void()> NotifyzfunReset;
