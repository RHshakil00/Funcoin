...
public:
    ...
    bool WriteZFUNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFUNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFUNSeed_deprecated(uint256& seed);
    bool EraseZFUNSeed();
    bool EraseZFUNSeed_deprecated();

    bool WriteZFUNCount(const uint32_t& nCount);
    bool ReadZFUNCount(uint32_t& nCount);
