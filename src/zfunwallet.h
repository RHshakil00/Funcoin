...
#ifndef FunCoin_ZFUNWALLET_H
#define FunCoin_ZFUNWALLET_H
...
public:
    CzfunWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFUN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFUN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FunCoin_ZFUNWALLET_H
