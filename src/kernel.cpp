...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZFUNStake(spend));
    } else {
        ...
        CFUNStake* FUNInput = new CFUNStake();
        FUNInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(FUNInput);
    }
