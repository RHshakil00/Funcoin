...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered funcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FUN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal funcoin:URI (funcoin:XyZ...?message=example)
            {
...
static void NotifyzfunReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzfunReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzfunReset.connect(boost::bind(NotifyzfunReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzfunReset.disconnect(boost::bind(NotifyzfunReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zfunTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
