...
#include "ui_zfuncontroldialog.h"
...
std::set<std::string> ZFUNControlDialog::setSelectedMints;
std::set<CMintMeta> ZFUNControlDialog::setMints;

ZFUNControlDialog::ZFUNControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFUNControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFUNControlDialog::~ZFUNControlDialog()
{
    delete ui;
}

void ZFUNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFUNControlDialog::updateList()
{
...
void ZFUNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFUNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFUN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFUNControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFUNControlDialog::GetSelectedMints()
{
...
void ZFUNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
