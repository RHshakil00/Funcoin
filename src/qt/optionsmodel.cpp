...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FUN);
    ...
    if (!settings.contains("nAnonymizeFunCoinAmount"))
        settings.setValue("nAnonymizeFunCoinAmount", 1000);

    nAnonymizeFunCoinAmount = settings.value("nAnonymizeFunCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFunCoinAmount"))
        SoftSetArg("-anonymizefuncoinamount", settings.value("nAnonymizeFunCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in funcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFunCoinAmount:
            return QVariant(nAnonymizeFunCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFunCoinAmount:
            nAnonymizeFunCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFunCoinAmount", nAnonymizeFunCoinAmount);
            emit anonymizeFunCoinAmountChanged(nAnonymizeFunCoinAmount);
            break;
        case CoinControlFeatures:
