...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

	{
		...
		for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
			...
			for (const CTxIn& txin : tx.vin) {
				...
				if (tx.IsZerocoinSpend()) {
					...
					//Priority = (age^6+100000)*amount - gives higher priority to zfuns that have been in mempool long
					//and higher priority to zfuns that are large in value
					...
					// zfun spends can have very large priority, use non-overflowing safe functions
		...
		while (!vecPriority.empty()) {
			...
			// double check that there are no double spent zfun spends in this block or tx
			if (tx.IsZerocoinSpend()) {
				int nHeightTx = 0;
				...
				//This zfun serial has already been included in the block, do not add this tx.
				if (fDoubleSerial)
					continue;
			}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		if (pblock->IsZerocoinStake())
			pwalletMain->zfunTracker->RemovePending(pblock->vtx[1].GetHash());
		return error("FunCoinMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("FunCoinMiner started\n");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		if (fProofOfStake) {
			...
			if (pblock->IsZerocoinStake()) {
				...
				if (!pwallet->GetZerocoinKey(bnSerial, key)) {
					LogPrintf("%s: failed to find zfun with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
					continue;
				}

				//Sign block with the zfun key
				if (!SignBlockWithKey(*pblock, key)) {
					LogPrintf("BitcoinMiner(): Signing new block with zfun key failed \n");
					continue;
				}
			}
		...
		LogPrintf("Running FunCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
