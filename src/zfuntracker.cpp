...
CzfunTracker::CzfunTracker(std::string strWalletFile)
{
...
CzfunTracker::~CzfunTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzfunTracker::Init()
{
...
bool CzfunTracker::Archive(CMintMeta& meta)
{
...
bool CzfunTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzfunTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzfunTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzfunTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzfunTracker::GetSerialHashes()
{
...
CAmount CzfunTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzfunTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzfunTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzfunTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzfunTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzfunTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzfunTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzfunTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzfunTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzfunTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzfunTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzfunTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzfunTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzfunTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzfunTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzfunTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzfunTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzfun from DB\n", __func__, listDeterministicDB.size());
    }
