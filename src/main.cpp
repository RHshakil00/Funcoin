...
/** Fees smaller than this (in uFUN) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zfun spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zfun is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zfun spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zfun without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zfun spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zfun spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfun");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zfun mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZFUNStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZFUNStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZFUNStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from FunCoin
...
void RecalculateZFUNSpent()
{
	...
	while (true) {
		...
		//Remove spends from zfun supply
...
bool RecalculateFUNSupply(int nHeightStart)
{
...
{
	// FunCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZFUNSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZFUNMinted();
		RecalculateZFUNSpent();
		RecalculateFUNSupply(Params().Zerocoin_StartHeight());
	}

	//Track zfun money supply in the block index
	if (!UpdateZFUNSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zfun supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFUNSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zfun spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zfun serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zfun stake block is less than allowed start height", __func__);

	if (CZFUNStake* zfun = dynamic_cast<CZFUNStake*>(stake)) {
		CBlockIndex* pindexFrom = zfun->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zfun stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zfun stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zfun->GetValue()));
		if (nChecksum200 != zfun->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zfun->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZFUN() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zfun fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> FUNInputs;
		std::vector<CTxIn> zfunInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zfunInputs.push_back(stakeIn);
			}
			else {
				FUNInputs.push_back(stakeIn);
			}
		}
		const bool hasFUNInputs = !FUNInputs.empty();
		const bool hasZFUNInputs = !zfunInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasFUNInputs)
					...
					for (CTxIn FUNIn : FUNInputs) {
						if (FUNIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zfun inputs.
			if (hasZFUNInputs) {
				for (CTxIn zFUNInput : zfunInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zFUNInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfun");
...
/** Compute what height to fun back to with the CBlockIndex::pskip pointer. */
int static inline GetSkipHeight(int height)
{
	if (height < 2)
		return 0;

	// Determine which height to fun back to. Any number strictly lower than height is acceptable,
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zfun mints and %d zfun spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// FunCoin: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
	// SPORK_14 was used for 90100 (v1.0.0+)
	     if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
	        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	// SPORK_15 is used for 90101 (v1.0.1+)
