...
CzfunWallet::CzfunWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfun seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFUNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFUNSeed_deprecated()) {
                    LogPrintf("%s: Updated zfun seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfun seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfun wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzfunWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFUNCount(nCountLastUsed);
    else if (!walletdb.ReadZFUNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzfunWallet::Lock()
{
    seedMaster = 0;
}

void CzfunWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzfunWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFUN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzfunWallet::LoadMintPoolFromDB()
{
...
void CzfunWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzfunWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzfunWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfunTracker->HasPubcoinHash(pMint.first)) {
...
bool CzfunWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFUN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfunTracker which also adds to database
    pwalletMain->zfunTracker->Add(dMint, true);
...
void CzfunWallet::SeedToZFUN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzfunWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzfunWallet::GenerateDeterministicZFUN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzfunWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzfunWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
